...
# -----------------------------------------------------------------------------------------------------------------------------------------
	...
	def dropLatent(self, latentCluster, pos, keyModifiers):
		...
		if keyModifiers == QtCore.Qt.ShiftModifier:
			print('<ImageViewer> perturbing latent at ({}, {}) with class {}'.format(
				gridCoords.x(), gridCoords.y(), latentCluster))
			sourceX = self.sampledLatentPos.x()
			sourceY = self.sampledLatentPos.y()
			output, undoCount = np.asarray(planet_manager.perturbLatent(
				gridCoords.x(), gridCoords.y(), sourceX, sourceY, alpha)._getvalue())
		else:
			print('<ImageViewer> dropped latent of class {} at ({}, {})'.format(
				latentCluster, gridCoords.x(), gridCoords.y()))
			output, undoCount = np.asarray(
				planet_manager.putLatent(gridCoords.x(), gridCoords.y(), latentCluster)._getvalue())
	...
	def replaceLatentRegion(self, startX, startY, width, height, sampleX, sampleY):#, x_target, y_target):
		...
		output, undoCount = np.asarray(planet_manager.pasteLatents(self.sampledLatent, startX, startY, width, height, sampleX, sampleY, mode)._getvalue())
		self.undoCountUpdated.emit(undoCount)
		self.updateImage(output)

	def undo(self):
		output, undoCount = np.asarray(planet_manager.undo()._getvalue())
	...
	def toggleMerging(self, showMerging):
		...
		if not showMerging and self._emptyUnmerged:
			output = np.asarray(planet_manager.getUnmergedOutput()._getvalue())
	...
	def toggleClusters(self, showClusters):
		...
		if showClusters and self._emptyClusters:
			...
			output = np.asarray(planet_manager.getClusterOutput()._getvalue())
			rect = self.getImageDims()
	...
	def refresh(self):
		...
		output, gridShape = np.asarray(planet_manager.getOutput()._getvalue())
		self.updateGridShape(np.asarray(gridShape))
		self.updateImage(output)

	def improveResults(self):
		print('<ImageViewer> improve')
		output = np.asarray(planet_manager.improveLatents()._getvalue())
		self.updateImage(output)
	...
	def randomize(self):
		...
		# output, gridShape, undoCount = planet_manager.randomizeGrid(h, w)._getvalue()
		output, gridShape, undoCount = planet_manager.randomizeGrid(h, w)
		self.undoCountUpdated.emit(undoCount)
		self.updateGridShape(np.asarray(gridShape))
	...
	def deadLeaves(self):
		...
		output, gridShape, undoCount = planet_manager.deadLeaves(h, w)._getvalue()
	...
	def setMergeLevel(self):
		...
		planet_manager.setMergeLevel(self.mergeLevel, self.latentSize)
		output, gridShape = np.asarray(planet_manager.getOutput()._getvalue())
		self.updateGridShape(np.asarray(gridShape))
	...
	def update(self):
		output, descriptorGrid, gridShape, undoCount = np.asarray(planet_manager.getUpsampled(self.guidanceMap)._getvalue())
		self.updateImage(output, fitToView=True)
		self.undoCountUpdated.emit(undoCount)
...
class MainWidget(QtWidgets.QWidget):
	...
	def createLatentPicker(self, widgetLayout):
		...
		# latent_images = np.asarray(planet_manager.getLatentImages()._getvalue())
		# latent_colors = np.asarray(planet_manager.getDominantClusterColors()._getvalue())
		latent_images = np.asarray(planet_manager.getLatentImages())
		latent_colors = np.asarray(planet_manager.getDominantClusterColors())
		# latent_images = np.asarray([[np.zeros((256, 256, 3)) for x in range(10)] for i in range(2)])
		# latent_colors = np.asarray([[256, 0, 0], [0, 256, 0]])
	...
	def setDataset(self):
		...
		# datasets, currentDataset = np.asarray(planet_manager.findDatasets()._getvalue())
		datasets, currentDataset = np.asarray(planet_manager.findDatasets())
		...
		planet_manager.initDataset(self.dataset)
		self.createLatentPicker(self.LatentPickerLayout)
...
from marsGAN_dummy_server import PlanetServer
...
if __name__ == '__main__':
	...
	# planet_manager, connected = getServer(ip=args.address, port=args.port)
	planet_manager = PlanetServer()
