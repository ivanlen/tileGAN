...
class PlanetServer:
	...
	def initDataset(self, datasetName):
		...
		networkPath = './test/netowrk.file'
		descriptorsPath = './test/desc.file'
		clustersPath = './test/clusters.file'
		kmeansPath = './test/kmean.file'
		annPath = './test/ann.file'
		metadata = './test/meta.file'
	...
	def initNetworks(self):
		...
		print('inResA', inResA)
		print('outResA', outResA)
		print('depthA', depthA)
		print('depthB', depthB)
		print('inSizeB', inSizeB)
		channels = 3
		size = 256
		self.outRes = int(np.log2(size))
		kwargs = {'in_res': inResA, 'out_res': outResA,
					'latent_depth': depthA, 'label_size': 0,
					'num_channels': channels, 'resolution': size}
		...
		# 	kwargs = {'in_res': inResB, 'out_res': outRes,
		# 	'latent_depth': depthB, 'latentSize': [None, depthB, inSizeB, inSizeB],
		# 	'label_size': 0, 'num_channels': channels, 'resolution': size}
	...
	def initLatentClusters(self, clustersPath):
		...
		self.latentImages = np.array([self.h_reds[0], self.h_greens[0], self.h_blues[0]]).astype('uint8')
		self.averageImages = np.array([x[:t_size, :t_size, :] for x in self.latentImages])
		...
		for i in range(self.numClusters):
			n = self.h_n_colors[i]
			self.latentClusters.append(np.array(np.arange(c_counter, c_counter + n)))
			self.latentCDFs.append([np.random.random() for _ in range(n)])
			c_counter += n
	...
	def loadLatents(self, descriptorsPath):
		...
		# IVAN: descriptors are the thumbnails of the images
		...
		example_descriptor = self.descriptorLookup[0]

		self.tSize = int(np.sqrt(len(example_descriptor)/3))
	...
	def pasteLatents(self, sampleLatent, targetX, targetY, targetW, targetH, sourceX, sourceY, mode='identical'):
		...
		if mode == 'similar':
			...
			cluster = self.latentClusters[clusterIndex] # IVAN: these are the index of the samples of each cluster
		...
		if repeat:
			...
			for x in np.arange(self.width):
				for y in np.arange(self.height):
					...
					self.clusterGrid[
					y*self.latentSize:(y+1)*self.latentSize,
					x*self.latentSize:(x+1)*self.latentSize] = clusters[y*w+x]
	...
	def calculateIntermediateLatents(self, latents):
		...
		# 	with self.sessionA.as_default():
		# 		intermediateLatents, _ = self.GsA.run(
		# 			latents,
		# 			in_res=2,
		# 			out_res=self.mergeLevel,
		# 			latent_depth=DEPTHS[0], # DEPTHS[0] = 512
		# 			minibatch_size=32,
		# 			num_gpus=1,
		# 			out_dtype=np.float32)
		out_int_shape = (512, 4, 4)
		intermediateLatents = np.array([np.random.random(out_int_shape) for _ in range(len(latents))]).astype(np.float32)
	...
	def getOutputFromLatents(self, latentList, updateUndoStack=True):
		...
		print('pad', pad)
		print('latentDepth', self.latentDepth)
		print('height', self.height)
		print('latentSize', latentSize)
		...
		for y in range(self.height):
			for x in range(self.width):
				self.intermLatentGrid[
				:, :, y * latentSize:(y + 1) * latentSize,
				x * latentSize:(x + 1) * latentSize
				] = intermLatents[y * self.width + x, :, pad:pad + latentSize, pad:pad + latentSize]
	...
	def putLatent(self, posX, posY, clusterIndex, fromSamples=False, useCDF=True, updateUndoStack=True):
		...
		if fromSamples and self.clusterSamples is not None:
			...
			randLatentIdx = cluster[randIdx] # this is the index of the latent
		...
		self.intermLatentGrid[
		:, :, y_start:y_end, x_start:x_end] = randIntermLatent[:, :, t:latentSize-b, l:latentSize-r]

		roi = 2 * latentSize
		returnImage = self.calculateOutputImage(
			self.intermLatentGrid,
			start=(max(y_start - roi, 0), max(x_start - roi, 0)),
			end=(min(y_end + 2 * roi, gridH), min(x_end + 2 * roi, gridW)),
			updateAll=False)
		t3 = time.time()
		return returnImage, self.undoCount
	...
	def putOnUndoStack(self, latentList):
		print(self.undoStack[1:].shape, np.expand_dims(latentList, axis=0).shape)
	...
	def calculateOutputImage(self, intermLatentGrid, start=(0, 0), end=(0, 0), updateAll=False):
		...
		if useChunks:  # process grid in chunks
			...
			# 	print(
			# 		'update network graph for different input shape {}x{}x{}'.format(s[1], chunkShape, chunkShape))
			...
			# _, self.output = self.GsB.run(intermLatentGrid, in_res=self.mergeLevel + 1, out_res=self.outRes,
			# 							  latent_size=[None, s[1], s[2], s[3]], latent_depth=s[1],
			# 							  minibatch_size=32, num_gpus=1, out_mul=127.5, out_add=127.5,
			# 							  out_dtype=np.uint8)
			# return np.squeeze(self.output)
